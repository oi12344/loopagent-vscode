...
const NODE_SCHEMA = {
	...
	properties: {
		...
		toolHints: { type: "array", items: { type: "string", minLength: 1 }, description: "Restrict this node to a subset of available tools. Valid values: browseSymbols, exploreCode, readFile, applyEdit, runCommand." },
	...
	return [
		{
			...
			description: [
				"Create and execute a computation graph in one call, then return every node result.",
				"",
				"PREFERRED API — use `nodes` (semantic, recommended for all new graphs):",
				"  nodes: [",
				"    { id: \"write\",  task: \"Implement the edit.\", role: \"executor\", timeoutMs: 60000 },",
				"    { id: \"review\", task: \"Reply APPROVED if correct, else list problems.\",",
				"      role: \"reviewer\", after: [\"write\"], reviews: [\"write\"], timeoutMs: 60000 },",
				"  ]",
				"  The engine derives review/fix loops automatically from `reviews`: if a reviewer returns",
				"  non-approval, the reviewed node re-runs. No cycles or condition expressions needed.",
				"  Use `after` for ordering, `contextFrom` to inject upstream output into a node's task.",
				"",
				"LEGACY API — use `initialNodes` + optional `resolvers` / `cycles` (compatibility only).",
				"  Use only when you need conditional expressions or resolver fanout the semantic API cannot express.",
				"",
				"EXPRESSION LANGUAGE (legacy `condition.expression` and `cycles[].exit.breakWhen[].value` only).",
				"This is NOT JavaScript. Only these forms are valid; anything else fails the whole call:",
				"  <nodeId>.content                     — that node's output text",
				"  <nodeId>.status                      — \"completed\" | \"failed\" | \"cancelled\"",
				"  <nodeId>.content.includes('text')    — substring test, single quotes required",
				"  !<expr>  |  <expr> === 'v'  |  <expr> !== 'v'  |  <expr> >= N  |  <exprA> && <exprB>",
				"  $globalKey                           — a value written by another node's exportTo",
				"NOT supported: nodes.get(), optional chaining (?.), .length, .match(), template literals, ||.",
				"",
				"NODE IDS: every id referenced in dependsOn, after, reviews, condition.expression,",
				"cycles[].from, cycles[].to MUST be declared in this call. Referencing undeclared ids is the most common failure.",
				"",
				"WRITE NODES REQUIRE A CONTRACT: any node with role=executor, or sideEffect other than \"none\",",
				"MUST declare outputContract, and its task MUST instruct the node to emit exactly that output.",
				"  Example: { id: \"write\", role: \"executor\",",
				"             task: \"Apply the edit and reply with JSON {filesChanged, summary}.\",",
				"             outputContract: { requiredFields: [\"filesChanged\", \"summary\"] } }",
				"Read-only roles (explorer/reviewer/planner) do not require a contract.",
			].join("\n"),
			inputSchema: {
				type: "object",
				properties: {
