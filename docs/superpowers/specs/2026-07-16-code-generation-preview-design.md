...
> **⚠️ 已被取代（2026-07-25）**：本文档描述的"预览 Diff → 阻塞等待用户确认 → 应用"流程已被替换为"直接应用 → 事后通知 → 用户可撤销"架构。详见 [2026-07-25-diff-view-enhancements-plan.md](../plans/2026-07-25-diff-view-enhancements-plan.md)。以下内容保留作历史记录，`applyEdit` 工具契约（`EditOperation` 结构）本身未变，但审阅/应用流程小节已过期。
