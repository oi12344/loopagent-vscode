...
`readFile` 默认最多返回 20,000 个字符；超过时截断并明确告知模型应使用更窄的行区间。行号从 1 开始，`startLine` 与 `endLine` 必须同时是正整数且 `startLine <= endLine`。
...
所有 Diff 打开后，Host 使用一个模态 `showInformationMessage` 提供“应用全部”和“取消”。用户关闭提示等同于取消。确认前重新读取所有受影响源文件；任一快照变化则拒绝整个提案，并把冲突说明返回模型。`workspace.applyEdit` 返回 `false` 时报告失败，不自动重试或部分补救。
...
- `readFile` 对完整内容、行区间、无效路径和超出工作区路径的处理。
- `applyEdit` 对 `replace`、创建、重命名、删除和多操作提案生成正确的虚拟 Diff 内容。
- 用户取消、关闭确认框、快照冲突和 `WorkspaceEdit` 返回 `false` 时均不改文件。
