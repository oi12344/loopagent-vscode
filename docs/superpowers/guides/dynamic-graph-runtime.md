...
系统提示要求模型按任务形状自行判断：单一、定义清晰、证据能在一个上下文内装下的任务（例如一次调用链问答、单文件编辑）直接用直接工具处理；只有当任务真正需要多路独立、可并行的探索并在之后汇总复核，或超出单次工具调用预算时，才调用 `runDynamicGraph`。这一判断依据 Anthropic《Building Effective AI Agents》和《When to use multi-agent systems》的指导：默认用最简单的方案，只在确有需要时才引入 workflow/多 Agent 编排。
